    ...
    for filename in sorted(os.listdir(model_path), key=lambda x: int(x.split('_')[1].split('.')[0])):
        if filename.endswith(".png"):
            ...
            try:
                ...
                analysis = DeepFace.analyze(image_path, actions=['gender', 'race'], enforce_detection=True)
                ...
                for face in analysis:
                    ...
                    filtered_race_probs = {race: prob for race, prob in race_probs.items() if race in ['asian', 'white', 'black', 'latino hispanic']}
                    race = max(filtered_race_probs, key=filtered_race_probs.get, default=None)
...
individual_results_file = "./sample_deepface.csv"  # Save directly to this path
